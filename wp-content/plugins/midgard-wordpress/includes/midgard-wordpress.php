 ...
 * @subpackage Midgard_WordPress/includes
 * @author     Firefly Interactive <info@fi.net.au>
 */
class Midgard_WordPress extends Midgard_Plugin_Base {
	 ...
	 * @since    1.0.0
	 * @access   protected
	 * @var      Midgard_WordPress_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	...
	public function __construct($plugin_name, $version) {
		...
		// plugin type strings
		$this->feed_type_label = __('WordPress', 'midgard-wordpress');
		$this->feed_type = 'wordpress';
	...
	public static function getAuthTokens() {
		$opt   = get_option( 'midgard_wordpress_settings' );
		$instances = array();
		if( $opt && isset($opt['wp_auth_tokens']) ) {
			$instances = json_decode($opt['wp_auth_tokens'], true);
		}
		return $instances;
	}

	/**
	 * Get a single stored auth token
	 */
	public static function getAuthToken($id) {
		$instances = self::getAuthTokens();
	 ...
	 *
	 * - Midgard_WordPress_Loader. Orchestrates the hooks of the plugin.
	 * - Midgard_WordPress_i18n. Defines internationalization functionality.
	 * - Midgard_WordPress_Admin. Defines all hooks for the admin area.
	 * - Midgard_WordPress_Public. Defines all hooks for the public side of the site.
	 *
	...
	private function load_dependencies() {
		 ...
		 */
		require_once $this->plugin_base . 'plugins/plugin-wordpress-data.php';

		/**
		 * The class responsible for customising the admin area UI
		 */
		require_once $this->plugin_base . 'plugins/plugin-wordpress-ui.php';
	...
	private function define_admin_hooks() {
		...
		// data handler
		$data = new Midgard_WordPress_Data();
		...
		// UI handler
		$customise_ui = new Midgard_WordPress_UI($this->feed_type, $this->feed_type_label);
