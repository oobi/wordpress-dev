...
Class Gecka_Submenu_Submenu {
	...
	private function Build($Options = null) 
	{
		...
		// current page's parent is top level element
		else if( $submenu === 'parent' ) {
			global $post, $wp_query;
			
			if( ( is_a($post, 'stdClass') || is_a($post, 'WP_Post') ) && (int)$post->ID ) {
			 	if( $submenu = $this->get_ancestor ( $wp_query->get_queried_object_id(), $menu_items, $_type) )
			 		$submenu = $submenu->ID;		 	
			}
     ...
     */
	private function get_ancestor ($postID, $menu_items, $type='post_type')  {
