...
class Midgard_Push_Notifications_Options_Page {
	 ...
	 * Tab settings
	 */
	private $tab_slug = 'push-notifications';
	private $tab_title = 'Push Notifications';
	...
	public function __construct() {
		// default JWT URL Key
		$default_key = 'midgard-push-notifications';

		// Set up variables
		$this->settings_page = 'midgard-push-notifications-settings-page';
		$this->config = array(
			'settings_key'  => 'midgard_push_notifications_settings',		//e.g. 'mailchimp_settings
			'page_title'  	=> 'Midgard Push Notifications Settings',		//e.g. 'MailChimp Settings
			'menu_title'  	=> 'Midgard Push Notifications',				//e.g. 'MailChimp Settings
			'menu_slug'  	=> 'midgard_push_notifications_settings'		//e.g. 'mailchimp_settings
		);
	...
	public function settings_tab_content() {
		if( Midgard_Options_Page::$current_tab == $this->tab_slug) {
			settings_fields( 'midgard_push_notifications_group' );		// Option group
	...
	public function page_init() {
		...
		add_settings_section(
			'midgard_push_notifications_group', 							// ID
			'JWT Settings', 								// Title
			array(), 										// Callback
		...
		/* REGISTER SETTINGS */
		register_setting(
			'midgard_push_notifications_group', 					// Option group
		...
		add_settings_field(
			...
			$this->settings_page, 				            // Page
			'midgard_push_notifications_group'					            // Section ID
		);
	...
	public function sanitize( $input ) {
		...
		if( isset( $input['jwt_key'] ) ) {
			...
			if( empty($value) ) {
				$value = 'midgard_push_notifications';
			}
	...
	public function jwt_key_callback() {
		...
		_e('This defines the URL key you will use to retrieve the token (JWT) value.', 'midgard-push-notifications');

		echo '<br>' . __('To generate a JWT:', 'midgard-push-notifications') . '<code>'  . get_site_url() . '?' . $this->options['jwt_key'] . '</code>';
		echo '<br>' . __('Return format:', 'midgard-push-notifications') . '<code>'  . get_site_url() . '?' . $this->options['jwt_key'] . '=XXX</code>';
...
if( is_admin() ) {
	$options_page = new Midgard_Push_Notifications_Options_Page();
}
