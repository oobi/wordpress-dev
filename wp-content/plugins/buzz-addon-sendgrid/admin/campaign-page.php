 ...
 *
 */
class Buzz_Newsletter_Sendgrid_Campaign_Page
{
	 ...
	 */
	static function enqueue_custom_scripts() {
		...
		wp_enqueue_script('campaign_monitor_admin_js', ( $plugin_base . "/admin/js/buzz-addon-sendgrid-admin.js"), array('jquery'), $script_version, true);
	...
	public function ff_newsletter_campaign_management_page() {
		...
		$api_key			= $cm_settings['sendgrid_api_key'];
		$api_key_is_invalid = $cm_settings['sendgrid_api_key_valid'] == 'false' ? TRUE : FALSE;
		$default_from_name 	= $cm_settings['sendgrid_from_name'];
		$default_from_email	= $cm_settings['sendgrid_from_email'];
		$default_reply_email= $cm_settings['sendgrid_reply_email'];
		...
		<div class="wrap">
			...
			if( !isset( $api_key ) || empty( $api_key ) ) : ?>
				<div id="api-key-not-set" class="error">
					...
					<p>The <b>API Key</b> must be set before you can manage your Email Campaigns. Set the <b>API Key</b> on the
						<a href="<?php echo get_admin_url( get_current_blog_id(), '/options-general.php?page=sendgrid_settings' ) ?>">Sendgrid Settings</a> page before continuing.</p>
				</div>
				<?php return;
			...
			<h2 class="nav-tab-wrapper">
				<a href="?post_type=newsletter&page=buzz-sendgrid&tab=current" class="nav-tab <?php echo $active_tab == 'current' ? 'nav-tab-active' : ''; ?>">Current Campaigns</a>
				<a href="?post_type=newsletter&page=buzz-sendgrid&tab=create" class="nav-tab <?php echo $active_tab == 'create' ? 'nav-tab-active' : ''; ?>">Create a Campaign</a>
			</h2>
