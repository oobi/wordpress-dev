...
class ExceptionThrower
{
	...
	static function Start($level = null)
	{
		...
		set_error_handler(array("FF\LogicalDoc\ExceptionThrower", "HandleError"), $level);
	}
	...
	static function HandleError($code, $string, $file, $line, $context)
	{
		...
		// if the error relatest to an HTTP request
		// attempt to get the first element which will be the response code (e.g. 401 )
		if( isset( $context['http_response_header'] )
				&& is_array( $context['http_response_header'] )
				&& count(  $context['http_response_header'] ) ) {
			$message = $context['http_response_header'][0];
		}
