<?php
namespace Firefly\Buzz\Customizer;
...
class Customizer {
	...
	public function __construct() {
		...
		if( ! isset( Config::get('config')['buzz_customizer'] ) || empty( Config::get('config')['buzz_customizer'] ) ) {
			return;
		}
	...
	private function setup_fields( ) {
		...
		$config = Config::get('config')['buzz_customizer'];
		if( $config['colors'] ) 				include 'controls/_colors.php';
		...
		if( $config['add-ons'] && class_exists('Buzz_Addon_Email_View') )
												include 'controls/custom-css-email.php'; // Only show if Email View addon plugin is enabled
		...
		if( $config['add-ons'] && class_exists('Buzz_Addon_Email_View') )
												include 'controls/addons/email-view.php'; // Only show if Email View addon plugin is enabled
