...
class Gallery
{
	...
	public function shortcode_atts_gallery( $out, $pairs, $atts ) {
	    $atts = shortcode_atts( Config::get('config')['gallery_attributes'] , $atts );
	...
	public function post_gallery( $output, $attr )
	{
		...
		$gallery_settings = Config::get('config')['gallery_settings'];
    ...
    $size_class = $gallery_settings['thumbnail_size'] ? $gallery_settings['thumbnail_size'] : tag_escape( $atts['size'] );
		...
		foreach ($attachments as $id => $attachment) {
		    ...
		    $output .= "<figure class='gallery-item {$link_class}'>\n";
			...
			$icon = "<span class='fal fa-plus fa-2x'></span>";
            $output .= "<div class='gallery-item__overlay d-flex justify-content-center align-items-center'>{$icon}</div>";
		    $output .= "</figure>\n";
		}
