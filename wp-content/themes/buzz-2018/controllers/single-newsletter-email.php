...
if ($newsletter) {
	...
	$context['config']['featured']['more']['label']			= Customizer::get_theme_mod($email_prefix . 'more_label');
	...
	$context['config']['index']['more']['label']		= Customizer::get_theme_mod($email_prefix . 'more_label');
...
ob_start();
Timber::render('email/base.twig', $context);
$html = ob_get_clean();
...
if (array_key_exists('buzz-email-view', $wp_styles->registered)) {
	...
	$response = wp_remote_get($email_css_url, array('timeout' => 5));
...
if (! empty($injected)) {
	$html = preg_replace_callback(
		'@<body[^>]*>@',
		function ($matches) use ($injected) {
			return $matches[0] . $injected;
		},
		$html
	);
...
$fonts = Customizer::get_theme_mod('buzz_font_urls');

if (! empty($fonts)) {
	$font_css = '';
	foreach ($fonts as $font) {
		$font_css .= "@import url('" . esc_url_raw($font['src']) . "');\n";
	}
	$html = preg_replace_callback(
		'@<head[^>]*>@',
		function ($matches) use ($font_css) {
			return $matches[0] . "\n" . '<style id="buzz-custom-fonts">' . $font_css . '</style>';
		},
		$html
	);
}
