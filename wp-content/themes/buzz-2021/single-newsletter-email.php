...
if( $newsletter ) {
	...
	/* override hero setting */
	$context['hero']['show'] 		= Customizer::get_theme_mod( $email_prefix . 'hero_show' );
	...
	$context['articles']['featured'] 	= Articles::pluck( $articles, $max, ['ff_featured_article'], false );
	...
	// template
	$context['config']['featured']['template']				= $featured_template['slug'] ?? 'image-above.html.twig';
...
$fonts = get_theme_mod('buzz_font_urls', []);

if( !empty( $fonts ) ) {
	$font_css = '';
	foreach( $fonts as $font ) {
		$font_css .= "@import url('" . $font['src'] . "');\n";
	}
	$html = preg_replace( '@<head[^>]*>@', '$0' . "\n" . '<style id="buzz-custom-fonts">' . $font_css . '</style>', $html );
}
