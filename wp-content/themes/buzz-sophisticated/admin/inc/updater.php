<?php
/* Load Class. */
$buzz_sophisticated_theme_updater = new Buzz_Sophisticated_Theme_Updater();

/**
 * Updater Class
**/
class Buzz_Sophisticated_Theme_Updater{
	...
	public function __construct() {
		...
		/* Updater Config */
		$this->config = array(
			'server'  => 'http://wpupdate.fi.net.au/',
			'type'    => 'theme',
			'id'      => 'buzz-sophisticated',
			'api'     => '1.0.0',
			'post'    => array(
				'site' => get_site_url()
			),
	...
	public function get_data( $action, $plugin = '' ){
		...
		$scheme = parse_url( $this->config['server'],  PHP_URL_SCHEME );
		$server = set_url_scheme( $this->config['server'], 'http' );		
		$url = $http_url = add_query_arg( $url_args, $server );		
		
		if ( $scheme == 'https' && $ssl = wp_http_supports( array( 'ssl' ) ) ){
